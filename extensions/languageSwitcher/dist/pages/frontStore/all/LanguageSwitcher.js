...
export default function LanguageSwitcher({ switchLanguageApi, currentLanguage, translations: translationsJson, supportedLanguages }) {
    ...
    return (React.createElement("div", { className: "language-switcher flex items-center gap-1" }, supportedLanguages.map((lang, index) => (React.createElement(React.Fragment, { key: lang.code },
        index > 0 && (React.createElement("span", { className: "text-muted-foreground text-xs select-none" }, "/")),
        React.createElement("button", { type: "button", onClick: () => handleSwitch(lang.code), className: `text-xs font-medium px-1 py-0.5 rounded transition-colors cursor-pointer ${lang.code === currentLanguage
                ? 'text-primary font-bold underline underline-offset-2'
                : 'text-muted-foreground hover:text-foreground'}`, "aria-label": `Switch to ${lang.label}`, "aria-current": lang.code === currentLanguage ? 'true' : undefined }, lang.label))))));
}
export const layout = {
