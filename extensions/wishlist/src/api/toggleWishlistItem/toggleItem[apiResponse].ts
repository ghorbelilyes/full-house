import { select, insert, del, update } from '@evershop/postgres-query-builder';
  ...
  try {
    ...
    if (!product_id) {
      ...
      return response.json({
        error: { status: 400, message: 'product_id est requis' }
      });
    }
    ...
    if (existing) {
      ...
      await del('wishlist_item')
        .where('wishlist_item_id', '=', existing.wishlist_item_id)
        .execute(pool);
